...
def compare_positions(data_exchange, theo_positions: Dict, real_positions: Dict, account_key: str, tolerance: float = 0.1, processor=None) -> Tuple[Dict, List[str]]:
    """
    ...
    for asset, theo_pos in theo_positions.items():
        ...
        price = None
        if processor and data_exchange in processor.price_cache and asset in processor.price_cache[data_exchange]:
            price_info = processor.price_cache[data_exchange][asset]
            price = price_info['price']
        else:
            logger.warning(f"No price available for {asset} in theo position on {data_exchange}, falling back to quantity comparison")
    ...
    for asset in real_positions:
        if asset not in theo_positions:
            ...
            if processor and data_exchange in processor.price_cache and asset in processor.price_cache[data_exchange]:
                ...
                logger.warning(f"No price available for {asset} in real position on {data_exchange}, falling back to quantity comparison")
            real_amount = abs(real_qty * price) if price is not None else None
            is_dust = False
            is_mismatch = False
